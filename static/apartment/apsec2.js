...
const i_class_val = ['fa-solid fa-wifi' , 'fa-solid fa-shop-lock' , 'fa-solid fa-lightbulb' , 'fa-solid fa-tv']
...
__SYD.apsec2_apartment_features_amenities = () =>{
    const render_el = () =>{
        ...
        apartmentFeature.forEach((content,i) => {
            element.push(
                __SYD.ssec4_all_services_grid_design_tabs_ind_tab({title:content,i_class:i_class_val[i]})
            )
        });
...
__SYD.ssec4_all_services_grid_design_tabs_ind_tab = ({title = 'def title' , i_class = ''} = {}) =>{
    return __c(
        ...
        [
            __SYD.iconify_30px({attr:i_class,clr:'#fff'}),
