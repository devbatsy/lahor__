...
__SYD.hsec3_video_container = () =>{
    return __c(
        'div',
        {
            style:__sC['hsec3_video_container']({method:'add',style:{background:'transparent',opacity:__p(['hsec3_video_container' , 'animate'],false) ? '1' : '0'}}),
            class:__p(['hsec3_video_container' , 'animate'],false) ? 'topSlide_animate' : ''
        },
        [
            __SYD.text_lib_type_h1_glob({cnt:"Lahor Luxury furnished and serviced apartments in Abuja",size:__p(['hsec3_video_container','font'],'40px'),clr:'#b99d75',w:'700',inject:"text-transform:capitalize;text-align:center;max-width:900px;padding:5px 15px;"}),
            ...
            __c(
                'div',
                {
                    style:'height:fit-content;width:100%;position:relative;display:flex;justify-content:center;align-items:center;'
                },
                [
                    __c(
                        'video',
                        {
                            style:`width:${__p(['hsec3_video_container','font'],'40px') === '40px' ? '80%' : '100%'};max-height:calc(100vh - 80px);`+__sC['border-rad-15'](),
                            class:'thin_border',
                            // controls:true,
        ...
        ],
        {
            createState:{
                ...
                state:{font:'40px',animate:false,init_count:() =>{
                    const state = __g('hsec3_video_container');
                    state.animate = true;
                    __u('hsec3_video_container' , {type:'a' , value:state});
                }}
            },
            mediaQuery:{
